...
function in_superstring(input, value) {
    return [value.indexOf(input) !== -1 ? 'success' : 'failure', []];
}
...
function matches_environment(input, value) {
    ...
    if (value.transform) {
        environmentValue = value.transform.reduce(function(ev, transformer) {
            if (typeof transformer === 'string') {
                return _VALUE_TRANSFORMERS[transformer](ev);
            } else {
                return _VALUE_TRANSFORMERS[transformer.type](ev,
                                                             transformer.value);
            }
        }, environmentValue);
    }
